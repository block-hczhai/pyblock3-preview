...
# E dmrg (4x4) = -15.476961593500 (approx)
...
class MERAState:
    ...
    def get_disentanglers(info_low, low=0, high=1, dtype=float):
        return SparseFermionTensor.random(
    ...
    @staticmethod
    def get_hamil_terms(topology, basis, dtype=float):
        terms = []
        ca_op = MERAState.c_operator(1, basis, 0, dtype=dtype)
        da_op = MERAState.d_operator(1, basis, 0, dtype=dtype)
        cb_op = MERAState.c_operator(-1, basis, 0, dtype=dtype)
        db_op = MERAState.d_operator(-1, basis, 0, dtype=dtype)
        u_op = MERAState.u_operator(basis, 0, dtype=dtype)
...
h_terms = MERAState.get_hamil_terms(topology, hamil.basis)
