...
class SliceableTensor(np.ndarray):
    ...
    def __repr__(self):
        ...
        p = np.asarray(self)
        return "\n".join("%10s %r" % (ix, p[tuple(ix)]) for ix in idx if p[tuple(ix)] != 0)
    ...
    def density(self):
        ...
        idx = np.indices(self.shape).reshape((self.ndim, -1)).transpose()
        p = np.asarray(self)
        return len([0 for ix in idx if p[tuple(ix)] != 0]) / self.size
...
class SparseTensor(NDArrayOperatorsMixin):
    ...
    def quick_deflate(self):
        return SparseTensor(blocks=[b for b in self.blocks if b != 0])
