...
def measure_SZ(symmetry=None, flat=None, large=None):
    ...
    blocks = [SubTensor(data=dat, q_labels=q_lab) for dat, q_lab in block_dict.values()]
...
def ParticleNumber(symmetry=None, flat=None, large=None):
    ...
    blocks = [SubTensor(data=dat, q_labels=q_lab) for q_lab, dat in block_dict.items()]
    T = SparseFermionTensor(blocks=blocks, pattern="+-")
    if large:
        return T.to_flat().to_large()
    elif flat:
...
def creation(spin='a', symmetry=None, flat=None, large=None):
    ...
    for s1 in cre_map.keys():
        ...
        for os1 in output_s1:
            ...
            dat[ix2, ix1] += phase
    blocks = [SubTensor(data=dat, q_labels=qlab) for qlab, dat in block_dict.items()]
...
def vaccum(n=1, symmetry=None, flat=None, large=None):
    ...
    arr[index] = 1
    q_labels = (q_label, ) * n
    blocks = [SubTensor(data=arr, q_labels=q_labels)]
...
def get_sparse_exponential(T, x):
    ...
    for slab, datasets in data_map.items():
        ...
        for iblk in datasets:
            ...
            if isinstance(phase, np.ndarray):
                phase = phase.reshape(ied-ist, jed-jst)
            data[ist:ied,jst:jed] = iblk.data.reshape(ied-ist, jed-jst) * phase
        if data.size ==0:
            continue
        el, ev = np.linalg.eigh(data)
        s = np.diag(np.exp(el*x))
        ...
        for lq, (ist, ied, ish) in row_map.items():
            for rq, (jst, jed, jsh) in row_map.items():
                ...
                if abs(chunk).max()<SVD_SCREENING:
                    continue
                blocks.append(SubTensor(data=chunk, q_labels=q_labels))
