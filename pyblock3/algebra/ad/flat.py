...
def jax_pytree_node(cls):
    ...
    tree_util.register_pytree_node(cls, cls.pytree_flatten, cls.pytree_unflatten)
    return cls
...
class FlatSparseTensor(NDArrayOperatorsMixin):
    ...
    def pytree_flatten(self):
        traced = (self.data, )
        others = (self.q_labels, self.shapes, self.idxs, )
        return traced, others
    
    @classmethod
    def pytree_unflatten(cls, others, traced):
        q_labels, shapes, idxs = others
        return cls(q_labels=q_labels, shapes=shapes, data=traced[0], idxs=idxs)
    ...
    def _real(x):
        return x.__class__(q_labels=x.q_labels, shapes=x.shapes,
                           data=jnp.ascontiguousarray(x.data.real), idxs=x.idxs)
    ...
    def _imag(x):
        return x.__class__(q_labels=x.q_labels, shapes=x.shapes,
                           data=jnp.ascontiguousarray(x.data.imag), idxs=x.idxs)
            ...
            if ufunc.__name__ in ["matmul"]:
                ...
                if isinstance(a, numbers.Number):
                    shs, qs, data, idxs = b.shapes, b.q_labels, getattr(
                        jnp, ufunc.__name__)(a, b.data), b.idxs
                elif isinstance(b, numbers.Number):
                    shs, qs, data, idxs = a.shapes, a.q_labels, getattr(
                        jnp, ufunc.__name__)(a.data, b), a.idxs
                else:
                    return NotImplemented
            elif len(inputs) == 1:
                a = inputs[0]
                shs, qs, data, idxs = a.shapes, a.q_labels, getattr(
                    jnp, ufunc.__name__)(a.data), a.idxs
            else:
                return NotImplemented
        else:
            return NotImplemented
...
class FlatFermionTensor(FermionTensor):
    ...
    def from_flat(x):
        return FlatFermionTensor(
            odd=FlatSparseTensor.from_flat(x.odd),
            even=FlatSparseTensor.from_flat(x.even)
        )
    ...
    def _tensordot(a, b, axes=2):
        ...
        if isinstance(idx, int):
            for x in blocks:
                mask = np.ones(x.data.shape, dtype=x.data.dtype)
                if x.n_blocks != 0:
                    for i, j, q in zip(x.idxs, x.idxs[1:], x.q_labels[:, idx]):
                        if SZ.is_flat_fermion(q):
                            np.negative(mask[i:j], out=mask[i:j])
                x.data = x.data * mask
        else:
            for x in blocks:
                mask = np.ones(x.data.shape, dtype=x.data.dtype)
    ...
    def split(self, xidx, nodd):
        ...
        odata = jnp.concatenate([dt[st:ed]
                                for st, ed in zip(ix[mo], ix[mo + 1])])
        edata = jnp.concatenate([dt[st:ed]
