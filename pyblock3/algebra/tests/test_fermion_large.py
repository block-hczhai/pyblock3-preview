...
class TestU11(unittest.TestCase):
    def setUp(self):
        ...
        self.Tf = self.T.to_flat().to_large()
        self.symmetry = U11
        ...
        self.T1f = self.T1.to_flat().to_large()
        self.Tt1 = rand((bond,)*4, pattern="++--", dq=U11(0)).to_flat().to_large()
        self.Tt2 = rand((bond,)*4, pattern="++--", dq=U11(1,1)).to_flat().to_large()
    
    def test_skeleton(self):
        bond = self.bond
        dq0 = self.skeleton_test_dq
        T = rand((bond,)*4, pattern="++--", dq=dq0).to_flat().to_large()
    ...
    def test_global_flip(self):
        ...
        if fermion_setting.DEFAULT_FERMION:
            self.assertAlmostEqual((self.Tf.to_flat() + new_Tf.to_flat()).norm(), 0, 8)
        else:
            self.assertAlmostEqual((self.Tf.to_flat() - new_Tf.to_flat()).norm(), 0, 8)
    ...
    def test_transpose(self):
        ...
        self.assertAlmostEqual((new_T.to_flat() - new_Tf.to_flat()).norm(), 0, 8)
    ...
    def test_tensor_contract(self):
        ...
        outf = np.tensordot(Ta.to_flat().to_large(), Tb.to_flat().to_large(), axes=((2,3),(1,0)))
        self.assertAlmostEqual((out.to_flat() - outf.to_flat()).norm(), 0., 8)

        out = np.tensordot(Ta, Tb, axes=((2,3),(0,1)))
        outf = np.tensordot(Ta.to_flat().to_large(), Tb.to_flat().to_large(), axes=((2,3),(0,1)))
        self.assertAlmostEqual((out.to_flat() - outf.to_flat()).norm(), 0., 8)
    ...
    def test_tensor_svd(self):
        ...
        for dq in self.svd_dq_iterator:
            ...
            for absorb in [0,1,-1,None]:
                ...
                self.assertAlmostEqual((out.to_flat() - Tf.to_flat()).norm(), 0, 8)
    ...
    def test_qr(self):
        ...
        left_idx=[3,1]
        for dq in self.svd_dq_iterator:
            T = rand((bond,)*4, pattern="++--", dq=dq).to_flat().to_large()
            ...
            self.assertAlmostEqual((out.to_flat() - T.to_flat()).norm(), 0, 8)
